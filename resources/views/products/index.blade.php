                ...
                <div class="row">
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            @endcan
                            <a href="{{ route('products.export') }}" class="btn btn-success">تحميل كملف
                                Excel</a>
                            <label for="fileInput" class="btn btn-success" id="customFileLabel">ارفع ملف المنتجات
                            </label>
                            <form id="uploadForm" action="{{ route('products.import') }}" method="post"
                        ...
                        <div class="mb-3">
                            <form action="{{ route('products.search') }}" method="get">
                                <div class="input-group">
                                    <input type="text" class="form-control" placeholder="ابحث عن منتج" name="search">
                                        ...
                                        @forelse ($products as $medicine)
                                            <tr>
                                                ...
                                                <td> <img style="width: 50px; height: 50px;"
                                                        src="{{ asset('images/products/' . $medicine->productimage) }}">
                                                </td>
                                                <td>{{ $medicine->productname }}</td>
                                                <td>{{ $medicine->productprice }}</td>
                                                ...
                                                <td>
                                                    @can('delete-product')
                                                        <form action="{{ route('products.destroy', ['id' => $medicine->id]) }}"
                                                            method="post"
                                                            onsubmit="return confirm('هل انت متأكد من حذف المنتج ؟');">
                                                            @csrf
                                                            @method('DELETE')
